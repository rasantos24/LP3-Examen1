...
void Personaje::draw(SDL_Renderer* renderer)
{
            ...
            if(frame%100==0)
            {
                ...
                if(textura_actual_int>=(*vector_textura_actual_temp).size())
                {
                    ...
                    if(vector_actual_str != "walk_right" && vector_actual_str != "walk_left")
                    {
                        ...
                        if(vector_actual_str == "kick_right")
                            setAnimacion("right");

                        if(vector_actual_str == "kick_left")
                            setAnimacion("left");

                        if(vector_actual_str == "gancho_right")
                            setAnimacion("right");

                        if(vector_actual_str == "gancho_left")
                            setAnimacion("left");
                    }
                }
    ...
    {
//        SDL_RenderCopy(renderer,textura_actual_temp[0],NULL,&rect);
...
void Personaje::setAnimacion(string nombre)
{
    if(this->vector_actual_str != nombre)
    {
        ...
        if(vector_actual_str == "jump_right" || nombre == "jump_right")
        {

        }
        else if(vector_actual_str == "jump_left" || nombre == "jump_left")
        {
